...
try {
  ...
  console.error('⚠️ Could not initialize Firestore:', e.message);
}
...
app.get('/map', (req, res) => res.redirect('/dashboard'));
...
async function addMockDataEntry() {
  try {
    ...
    console.log(`✓ Mock: ${loc.name} | Chl: ${chlorophyll} | Risk: ${calcRiskLevel(chlorophyll)}`);
  } catch (error) {
...
app.post('/api/mock/start', async (req, res) => {
  if (isMockRunning) return res.json({ success: false, message: 'Mock already running' });
  try {
    await addMockDataEntry();
    mockInterval  = setInterval(addMockDataEntry, 10000);
    isMockRunning = true;
    res.json({ success: true, message: 'Mock started - every 10s' });
  } catch (error) {
...
app.post('/api/mock/stop', (req, res) => {
  if (!isMockRunning) return res.json({ success: false, message: 'Mock not running' });
  clearInterval(mockInterval);
  mockInterval  = null;
  isMockRunning = false;
  res.json({ success: true, message: 'Mock stopped' });
});

app.post('/api/mock/cleanup', async (req, res) => {
  try {
    const snapshot = await db.collection('sensor_data')
      .where('isMockData', '==', true).get();
    if (snapshot.empty) return res.json({ success: true, message: 'No mock data', count: 0 });
    const batch = db.batch();
    snapshot.docs.forEach(doc => batch.delete(doc.ref));
    await batch.commit();
    res.json({ success: true, message: `Deleted ${snapshot.size} entries`, count: snapshot.size });
  } catch (error) {
    res.json({ success: false, message: error.message });
  }
});
